...
class EFIT:
    ...
    def __init__(self, xGrid, yGrid, zGrid, tStep, dStep):
        ...
        self.GridShapeS = (3,3,xGrid+1,yGrid+1,zGrid+1)
        #materials property gird.  Initially 3 properties needed: density, Lame 1, Lame 2 plus the boundary conditions
        self.GridShapeP = (4,xGrid+1,yGrid+1,zGrid+1)
     
        self.GridPointsV = (xGrid)*(yGrid)*(zGrid)
        self.GridPointsS = (xGrid+1)*(yGrid+1)*(zGrid+1)
        ...
        #Assumed properties are density, Lame 1, Lame 2, and the boundary conditions
        self.Gp = np.zeros(4*self.GridPointsS,dtype="float32").reshape(*self.GridShapeP)
    ...
    def DeltaStress(self,x,y,z):
        ...
        if x == 0:
            ...
            Ds[0,0] = ((1/self.__ds) * ((Lame1+2*Lame2)*(self.Gv[0,x,y,z]-self.Gv[0,x-1,y,z])))
        elif y==0:
            Ds[0,0] = ((1/self.__ds) *
